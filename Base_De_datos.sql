...
BEGIN
	...
	INSERT INTO DISPONIBILIDAD_INVOLUCRADO_POR_FECHA_TRANSPORTE 
			(COD_INVOLUCRADO, COD_TRANSPORTE)
...
BEGIN
	...
	IF EXISTS (
		...
		INSERT INTO PLAZAS_OCUPADAS_CONDUCTOR_POR_FECHA_TRANSPORTE 
				(COD_CONDUCTOR, COD_TRANSPORTE, PLAZAS_DISPONIBLES, PLAZAS_OCUPADAS)
			SELECT NEW.COD_INVOLUCRADO, NEW.COD_TRANSPORTE, ipp.PLAZAS, 0
				FROM INVOLUCRADO_POR_PLANTILLA ipp
				WHERE 
...
CREATE TABLE IF NOT EXISTS PLAZAS_OCUPADAS_CONDUCTOR_POR_FECHA_TRANSPORTE (
    ...
    COD_TRANSPORTE INT UNSIGNED NOT NULL,
...
BEGIN
    ...
    SELECT inv.ID, NEW.ID, inv.NUMERO_PLAZAS, inv.COD_ROL
