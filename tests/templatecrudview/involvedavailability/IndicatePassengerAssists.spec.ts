...
test.describe('Indicate Passenger Assists', () => {
  ...
  test('indicate passenger assists', async ({ page }) => {
    const passengerAssists = new PassengerAssists(page);
    const passengerTdSelector = `#${PASSENGER_TABLE_ID} tbody tr:nth-child(1) td:nth-child(3)`;
    const assistanceIconSelector = passengerTdSelector + AbstractPassengerAssists.ASSISTANCE_ICON_SELECTOR;

    // Verify initial state
    const isInitialStateOk = 
      await passengerAssists.isEverythingOkWhenPassengerDoesNotAssist(passengerTdSelector);
    ...
    // Verify final state
    const isFinalStateOk = 
      await passengerAssists.isEverythingOkWhenPassengerAssistsAndNeedsTransport(passengerTdSelector);
